...
def eprank(game_mat: np.ndarray, num_players:int, num_iters:int):
    """Expectation Propagation Ranking Algorithm (Message Passing)

    :param game_mat: Game outcomes matrix of shape [num_games, 2] where the first column is the winning player id,
        and the second column is the loosing player id. (one player always wins)
...
def eprank_iter(game_mat: np.ndarray, num_players:int, num_iters:int) -> Iterable:
    """Expectation Propagation Ranking Algorithm (Message Passing)

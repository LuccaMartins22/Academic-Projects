...
def gen_posterior(A, W, D, K, alpha, gamma, num_gibbs_iters):
    # A's columns are doc_id, word_id, count
    swd = sparse((A[:, 2], (A[:, 1]-1, A[:, 0]-1))).tocsr()
    ...
    skd = np.zeros((K, D))  # count of word assignments to topics for document d
    swk = np.zeros((W, K))  # unique word topic assignment counts across all documents
    word_counts_by_id_topic_it = np.zeros((W, K, num_gibbs_iters+1))
    topic_counts_by_id_doc_it = np.zeros((K, D, num_gibbs_iters+1))
    ...
    word_counts_by_id_topic_it[:, :, 0] = np.copy(swk)[:, :]
    topic_counts_by_id_doc_it[:, :, 0] = np.copy(skd)[:, :]
    ...
    for iter in tqdm(range(num_gibbs_iters)):
        ...
        word_counts_by_id_topic_it[:, :, iter+1] = np.copy(swk)[:, :]
        topic_counts_by_id_doc_it[:, :, iter+1] = np.copy(skd)[:, :]

    return swk, sk, word_counts_by_id_topic_it, topic_counts_by_id_doc_it


def test_perplexity(B, W, D, K, swk, sk, alpha, gamma, num_gibbs_iters):
    Swd = sparse((B[:, 2], (B[:, 1]-1, B[:, 0]-1))).tocsr()
